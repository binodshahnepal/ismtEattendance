...
export const dbService = {
  ...
  async addModule(id, title, programId, stage, trimester, tutor, batchId, section = 'A') {
    if (isCloudActive()) {
      const { error } = await supabase
        .from('modules')
        .insert([{ id, title, program_id: programId, stage: parseInt(stage), trimester: parseInt(trimester), tutor, batch_id: batchId, section }]);
      if (!error) return true;
  ...
  async getStudentAttendanceHistory(studentId) {
    if (isCloudActive()) {
      const { data, error } = await supabase
        .from('attendance')
        .select('record_date, status, notes, enrollments!inner(student_id, module_id, modules(title))')
